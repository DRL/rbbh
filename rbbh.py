...
Options:
       -h --help                                show this
       -1, --input_dir1 <DIR>                   Input directory of proteome 1 (BLAST result, query FASTA)
       -2, --input_dir2 <DIR>                   Input directory of proteome 2 (BLAST result, query FASTA)
...
try:
    ...
    sys.exit("[X] Please install the docopt python library.")
...
def write_file(out_f, outprefix, header, string):
    ...
    print("[+] \t Writing file %s ..." % str(out_f))
    with open(str(out_f), 'w') as out_fh:
        if header:
            out_fh.write("%s\n" % (header))
        out_fh.write("%s\n" % "\n".join(string))
...
class AnalysisCollection():
    def __init__(self, proteomeObjs, outprefix):
        ...
        self.analyse_rbbhs(outprefix)

    def analyse_rbbhs(self, outprefix):
        ...
        rbbh_out_f = "%s.%s.%s.rbbh.txt" % (outprefix, EVALUE, QCOV)
        ...
        for idx, rbbh_fastaObj in enumerate(rbbh_fastaObjs):
            out_f = "%s.rbbh_%s.%s.%s.faa" % (outprefix, idx, EVALUE, QCOV)
...
if __name__ == "__main__":
    ...
    analysisCollection = AnalysisCollection(proteomeObjs, outprefix)
